...
Food recalls are routine. The FDA and USDA between them announce hundreds every year; in one two week window in September 2026 the openFDA enforcement feed alone carried 21 new food recalls, for Salmonella, Listeria, glass fragments and undeclared allergens. Every one of them lands on a retailer that is still selling the product.

What happens next is bad in two directions at once.

**Too late.** Retailers find out from the press release, which comes days after the manufacturer knew, or from the openFDA enforcement report, which comes weeks after that. Some products are pulled from a manufacturer's catalog with no notice ever filed. In the gap, the recalled units keep shipping, and customers who already ordered them get no warning.

**Too much.** A recall names lots, not products. "Lot 1226183" is the notice; "every unit of that noodle kit" is what the retailer pulls, because the shop's systems do not know which lot sits in which box. Stock that was never contaminated is destroyed, the product disappears from the storefront for weeks, and in-flight orders are cancelled with a form email.

Both failures are records problems. Nobody connects the notice to the catalog, the catalog to the lots on the shelf, and the shelf to the orders already placed. And when the regulator or the insurer asks what was done and when, the answer is reconstructed from Slack messages.

Sotería is the connection. It watches the agency feeds continuously, reads each notice down to the exact barcodes and lot codes, holds only those lots in the store, offers affected customers a substitute or a refund before the order ships, watches resale marketplaces for the held lots reappearing, and writes every step into a hash chain that can be verified later.

Full requirements: [`docs/prd.md`](docs/prd.md).

## What we built

Twelve services on a message bus, two web apps, one shared Shopify integration. Everything runs from one `docker compose up`, and the whole chain has been run end to end against a live Shopify store with a real FDA notice.
...
| `recall-extractor` | Go | 8086 | LLM extraction of products, barcodes, lots, hazard and allergens from a notice, validated by a deterministic guardrail. 1.000 precision and recall on barcodes and lots across 16 real notices |
...
| `rabbitmq` | | 5672 | The bus. Topology imported at boot from `contracts/` |

### Shopify integration

`libs/shopify` is the one client every service uses, with an in-memory fake that implements the same interface for tests and for running without credentials. The lot ledger lives in the store itself, as a `soteria.lots` metafield on each variant, so the store is the source of truth for which lot codes are on the shelf and how many units each has. A hold moves exactly those units from the selling location to a Quarantine location in one paired inventory adjustment, marks the lots held, tags the product `RECALL_LOT:<code>` and sets a "Verified Safe Lot" badge on the remaining stock. A release reverses every step. Whole-SKU holds, when no lot can be named, unpublish the product instead.

### Demo tooling

`tools/democtl` makes a run repeatable against the live store and the real broker:
...
Sixteen real notices are available to inject, from `services/recall-extractor/eval/cases.json`.
...
**Feed outages are visible.** Every ingestion service serves `/healthz`, which returns 503 after three consecutive failed polls, when the last success is older than three intervals, or when the broker connection drops, even while idle. `/metrics` is Prometheus text. The n8n workflow in `automation/` polls these and alerts.
...
**Verified against real systems.** The full chain has run over a real broker against the live store: a real FDA notice (H-1258-2026, undeclared fish) resolved at lot scope with confidence 0.972, 40 units of lot 1226183 moved to Quarantine with 60 left on sale, the rescue offer went to the customer with the order, five resale listings were flagged, and the dossier verified with nine events. The three agency feeds return real data from India, where the team is based. The extractor eval runs against 16 real notices with recorded responses.

## Running it

```sh
cp infra/.env.example infra/.env     # broker password required; Shopify, Groq, Slack, Resend optional
cd infra && docker compose up -d     # broker with topology, all services
...
| `services/` | The twelve services above, each with its own module, Dockerfile, README and tests |
